...
data_main_selected <- data_main_cleaned %>%
  ...
  select(
    ...
    PROMIS_PF_0_1mo = pf_114,
    PROMIS_PI_0_1mo = pi_115,
    PROMIS_Anxiety_0_1mo = as_116, 
    PROMIS_PF_1_3mo = pf_118,
    PROMIS_PI_1_3mo = pi_119,
    PROMIS_Anxiety_1_3mo = as_120, 
    PROMIS_PF_3_6mo = pf_122,
    PROMIS_PI_3_6mo = pi_123,
    PROMIS_Anxiety_3_6mo = as_124,
...
longitudinal_vars <- list(
  ...
  list(var = "PROMIS_PF_0_1mo", bad_value = 20, description = "PROMIS PF 0-1mo (low=bad)"),
  ...
  list(var = "PROMIS_PI_0_1mo", bad_value = 70, description = "PROMIS PI 0-1mo (high=bad)"),
  ...
  list(var = "PROMIS_Anxiety_0_1mo", bad_value = 70, description = "PROMIS Anxiety 0-1mo (high=bad)"),
  ...
  list(var = "RUST_Score_0to1", bad_value = 4, description = "RUST 0-1mo (low=bad, 4=worst)"),
...
missing_flag_cols <- grep("_IsMissing$", names(data_with_dummies), value = TRUE)

# Define clinical predictors
clinical_predictors <- c("Age", "BMI", "Smoking_Status", "ISS", "cci_score", "IsOpen")

# Define PROMIS predictors with their missingness flags
promis_predictors <- c("PROMIS_PF_0_1mo", "PROMIS_PI_0_1mo", "PROMIS_Anxiety_0_1mo",
                       "PROMIS_PF_1_3mo", "PROMIS_PI_1_3mo", "PROMIS_Anxiety_1_3mo",
                       "PROMIS_PF_3_6mo", "PROMIS_PI_3_6mo", "PROMIS_Anxiety_3_6mo")

# Define RUST predictors with their missingness flags
rust_predictors <- c("RUST_Score_0to1", "RUST_Score_1to3", "RUST_Score_3to6")

# Model 1: Baseline PoC (Clinical + PROMIS + Missingness Flags + AO dummies)
model1_data <- data_with_dummies %>%
  select(
    Nonunion_Label,
    all_of(clinical_predictors),
    all_of(promis_predictors),
    all_of(intersect(missing_flag_cols, paste0(promis_predictors, "_IsMissing"))),
    all_of(ao_dummy_cols)
  )

cat("  - Model 1 (Baseline PoC): ", ncol(model1_data), "columns (",
    ncol(model1_data) - 1, "predictors + outcome)\n")
cat("    Predictors: Clinical + PROMIS (all timepoints) + Missingness Flags + AO Classification\n")

# Model 2: Full PoC (Clinical + PROMIS + RUST + All Missingness Flags + AO dummies)
model2_data <- data_with_dummies %>%
  select(
    ...
    all_of(missing_flag_cols),
    all_of(ao_dummy_cols)
  )

cat("  - Model 2 (Full PoC): ", ncol(model2_data), "columns (",
    ncol(model2_data) - 1, "predictors + outcome)\n")
cat("    Predictors: Clinical + PROMIS (all timepoints) + 
    RUST (all timepoints) + All Missingness Flags + AO Classification\n\n")
# ------------------------------------------------------------------------------
# 8. Save Output
# ------------------------------------------------------------------------------

# ==============================================================================
# Tibial Fracture Nonunion Prediction Model Comparison
# ==============================================================================
...
prepare_data <- function(data) {
  X <- as.matrix(data[, -1])  # All columns except outcome
  y <- factor(data$Nonunion_Label, levels = c(0, 1), labels = c("Union", "Nonunion"))
  list(X = X, y = y)
}
...
train_elastic_net <- function(X_train, y_train, X_val = NULL, y_val = NULL,
  ...
  if (tune) {
    ...
    for (alpha in alpha_grid) {
      # Fit with cv.glmnet for lambda tuning
      cv_fit <- tryCatch({
        cv.glmnet(
          x = X_train,
          y = y_train,
          ...
          nfolds = 5,
          standardize = TRUE,
          maxit = 100000  # Handle convergence issues
        )
      }, error = function(e) NULL)

      if (!is.null(cv_fit)) {
        # Get AUC at lambda.1se (more regularized, better generalization)
    ...
    # Train with fixed hyperparameters (for final model)
    cv_fit <- cv.glmnet(
      x = X_train,
      y = y_train,
      family = "binomial",
...
# XGBoost with nested CV and class weights
train_xgboost <- function(X_train, y_train, X_val = NULL, y_val = NULL,
                          class_weights, tune = TRUE) {

  # Convert to binary labels
  y_binary <- ifelse(y_train == "Nonunion", 1, 0)
  ...
  # Prepare DMatrix
  dtrain <- xgb.DMatrix(data = X_train, label = y_binary, weight = sample_weights)
  ...
  if (tune) {
    ...
    # Sample from grid if too large
    if (nrow(param_grid) > 10) {
    ...
    for (i in 1:nrow(param_grid)) {
      ...
      if (!is.null(cv_result)) {
        ...
        current_auc <- max(cv_result$evaluation_log$test_auc_mean)
...
for (rep in 1:n_repetitions) {
  ...
  # Create stratified folds for this repetition
  ...
  for (fold in 1:n_outer_folds) {
    ...
    # Calculate AUCs
    roc1_en <- roc(y_test, pred1_en, levels = c("Union", "Nonunion"), direction = "<", quiet = TRUE)
    roc2_en <- roc(y_test, pred2_en, levels = c("Union", "Nonunion"), direction = "<", quiet = TRUE)
    roc1_xgb <- roc(y_test, pred1_xgb, levels = c("Union", "Nonunion"), direction = "<", quiet = TRUE)
    roc2_xgb <- roc(y_test, pred2_xgb, levels = c("Union", "Nonunion"), direction = "<", quiet = TRUE)
...
roc_m1_en_final <- roc(patient_predictions$true_label, patient_predictions$model1_elastic_pred,
                       levels = c("Union", "Nonunion"), direction = "<", quiet = TRUE)
...
y_binary <- ifelse(patient_predictions$true_label == "Nonunion", 1, 0)
...
create_calibration_plot <- function(y_true, y_pred, model_name, n_bins = 10) {
  # Convert to binary
  y_binary <- ifelse(y_true == "Nonunion", 1, 0)

...
# Fix 1: Change prepare_data function (around line 75-82)
# Prepare data matrices - KEEP Y NUMERIC FOR TRAINING
prepare_data <- function(data) {
  X <- as.matrix(data[, -1])  
  y <- data$Nonunion_Label  # Keep as numeric 0/1
  list(X = X, y = y)
}
...
train_xgboost <- function(X_train, y_train, X_val = NULL, y_val = NULL,
  ...
  if(length(unique(y_train)) < 2) {
    return(NULL)  # Can't train with single class
  }
